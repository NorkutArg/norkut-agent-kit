...
2. **Clasificar cada nota** en una de:
   - `cross-repo` → va al kit (`plugins/norkut-core/memory/`): afecta a más de un repo, a un contrato, a ownership, a infra, al workflow.
...
3. **Elegir archivo destino** según el tipo: `gotchas.md`, `modules.md`, `event-contracts.md`, `risks.md`, `workflow.md` o un ADR nuevo en `decisions/`.
4. **Reescribir** cada línea al formato: `- YYYY-MM · <origen: PR #, daily, incidente> · <hecho o decisión, una línea, sin opinión>`. ADRs: título, contexto, decisión, consecuencias, en < 30 líneas.
5. **Mostrar la propuesta** al usuario: tabla nota → destino → texto final. Esperar confirmación y ediciones.
6. **Abrir el PR**: branch `memory/<slug>`, commit en español, PR con la tabla del paso 5 como descripción, reviewer = owner del módulo afectado según `modules.md` (o Diego para cross-cutting).
